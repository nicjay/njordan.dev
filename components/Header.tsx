...
function NavItem({ href, text }: { href: string; text: string }) {
  ...
  return (
    <Link
      href={href}
      className={`border-emerald-700 p-4 hover:bg-slate-200 dark:border-orange-400 hover:dark:bg-slate-800 ${
        isActive
  ...
  return (
    <div className="flex justify-between">
      <Link href="/" className="group flex items-center gap-1">
        <svg className="h-10 w-10 rounded-lg dark:fill-white" viewBox="0 0 512 512">
        ...
        <div
          className={`hidden text-lg font-semibold text-slate-900 transition dark:text-slate-50 sm:flex ${
            router.asPath === '/' ? 'opacity-0' : 'translate-x-1 opacity-100'
          }`}
        >
          Nick Jordan
        </div>
